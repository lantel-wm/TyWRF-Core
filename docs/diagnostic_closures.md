...
D73 is complete, verified, pushed, and synchronized at commit `b73704f`. It
audited only the adapter/report boundary: adapter/helper/staging metadata stays
visible, report-only, and non-gate. Strict gate rejection happens before RMSE or
TC diagnostics are interpreted. Therefore an adapter, helper, staging, dry-run,
probe, diagnostic, oracle, or reference-copy artifact remains rejected even if
its field RMSE, storm-center, MSLP, or Vmax diagnostics would otherwise meet
thresholds. Such diagnostics can inform future producer work, but they cannot
...
D91 remains outside diagnostic closures. It is complete, verified, and pushed
at commit `2a2de69` (`Add wind advection form sensitivity`). Its scope was
residual localization, audit metadata/delta improvements, and default-off
advection-form sensitivity. D91 must not borrow closure metadata, use
reference-end truth, oracle sources, or reference-copy data. It must not invoke
pressure refresh, physics, or best-track nudging, lower d02 below `2 km`, or
justify any `00:20` validation progress.
...
The D91 wind-audit extension records advection-form metadata and explicit
baseline-delta statistics. Those metadata and delta reports remain diagnostic
context for failed wind candidates; they are not closure evidence and cannot be
used to satisfy a default validation gate.

D92 also remains outside diagnostic closures. Its allowed scope is a
tendency-decomposition audit and a standalone, default-off pressure-gradient
wind-tendency staging skeleton. D92 must not claim a validation pass, advance
to `00:20`, use reference-end/oracle data for model updates, reduce d02 below
`2 km`, introduce best-track nudging, or couple this round to pressure refresh,
physics, or any closure that patches pressure or storm diagnostics.
...
`34/34`. The D92 real audit smoke remains diagnostic-only: `U` and `V`
residual normalized RMSE values of about `0.1679774` and `0.1601626` are audit
measurements, not closure evidence and not default-gate evidence.

D93 remains outside diagnostic closures. Its allowed scope is selected-field,
default-off pressure-gradient wind-tendency opt-in wiring that may write only
`U` and `V`. The default `none` path must preserve existing output, and D93
must not write or repair `P`, `MU`, `PB`, or `PHB`. It must not invoke pressure
refresh, physics, best-track nudging, or any closure path that patches pressure
or wind metrics.

The D93 pressure-gradient option is sensitivity/staging only. A constant-alpha,
first-order pressure-gradient approximation is not WRF-exact and must not be
used as a diagnostic closure, oracle correction, or validation pass claim. D93
